#!/usr/bin/env python3
"""
Speech-to-text processor using Faster Whisper large-v3 model for best accuracy.

The script expects an audio file (e.g. /tmp/recorded_audio.wav) as an argument.

Usage: python3 speech_to_text_large.py <audio_file>
...
def transcribe_audio(audio):
    ...
    try:
        logging.info("Loading Whisper large-v3 model (this will take significantly longer and use more memory)...")
        start_load = time.time()
        model = WhisperModel("large-v3", device="cpu", compute_type="int8")
        load_time = time.time() - start_load
        logging.info(f"Model loaded in {load_time:.2f} seconds (~3GB RAM)")
...
def main():
    ...
    # Check arguments
    if len(sys.argv) < 2:
        print("Usage: python speech_to_text_large.py <audio_file>")
        sys.exit(1)
